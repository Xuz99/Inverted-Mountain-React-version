...
const inputStyle =
  "appearance-none mt-5 text-md md:text-2xl fw-300 text-center font-in-input-pink w-4/5 px-3 py-1 xl:p-5 mb-10 focus:outline-none focus:bg-gray-800 bg-gray-800 rounded-full font-in-border-blue border-4 tracking-widest";
const buttonStyle =
  "hover:bg-blue-300 hover:text-white xl:text-5xl text-2xl text-blue-400 tracking-track-02 xl:px-8 px-4 py-2 mx-auto mb-10 -my-5 text-center lato fw-700 font-in-border-blue border-2 cursor-pointer";
...
const requried = (value) =>
  value && value.trim() !== "" ? undefined : "Requried *";

const Newsletter = () => {
  const [statusInput, validationAttrInput] = useValidation([
    requried,
    isValidEmail,
  ]);
    ...
    <Fragment>
      <section className="mt-56 border font-in-border-blue border-current xl:mx-64 mx-1">
        <div className="text-center">
          <img
            className="mx-auto my-10 w-64 sm:w-auto"
            src={newletter_image}
            alt="A man or alien beaming out the ether"
          />
          <h2 className="text-4xl xl:text-6xl font-in-blue tracking-widest">
            STAY TUNED
          </h2>
          <h3 className="text-xl xl:text-4xl font-in-subtitle-blue tracking-widest">
            with our newsether
          </h3>
          <img
            src={dots_image}
            className="text-xl xl:text-3xl mx-auto my-10"
            alt="3 seperating dots of ether"
          />
        </div>
        ...
        <Form
          className={formStyle}
          initialState={{ email: "" }}
          onSubmit={(state) => EmailCatch(state)}
        >
          {statusInput.error && (
            <label className={labelErrStyle}>{statusInput.error}</label>
          )}
          <Input
